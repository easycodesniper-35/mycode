...
def stitch(image_path,k):
    ...
    image1 = url_to_image(path1)
    image2 = url_to_image(path2)
...
def hist_match(image_path,k):
    ...
    src_img = cv2.cvtColor(url_to_image( srcimg_name ), cv2.COLOR_RGB2GRAY)
    ref_img = cv2.cvtColor(url_to_image( refimg_name ), cv2.COLOR_RGB2GRAY)
...
def fusion(image_path,k):
    ...
    if int(k) == 0:
        ...
        # cv2.imwrite('fusion.jpg',self._fusionImage)
        return image_out
...
if __name__ == "__main__":
    ...
    # input_path = argv[1]
    i = argv[1]
    input_path = 'http://e.hiphotos.baidu.com/image/pic/item/10dfa9ec8a1363279e1ed28c9b8fa0ec09fac79a.jpg'
    # input_path = 'D:\Work\OpenCV\\45.jpg'
    # output_path = argv[2]
    function = argv[2]
    parameter = argv[3]
    output_image = Fun_map[function](input_path,parameter)
    # byte_img = image_to_bytes(output_image)
    base64_img = image_to_base64(output_image)
    out = base64_to_image(base64_img)
    cv2.imwrite('test.jpg',out)
    # print(base64_img)
    # print(byte_img)
    # print(byte_img)
    # cv_imwrite(output_path,output_image)
    end = time.time()
    print(end-start)