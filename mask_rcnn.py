...
def drawBox(frame, classId, conf, left, top, right, bottom, classMask):
    ...
    try:
        area = str(areaCal(contours))

        print(label,areaCal(contours))

        labelSize, baseLine = cv.getTextSize(area, cv.FONT_HERSHEY_SIMPLEX, 0.5, 1)
        top = max(top, labelSize[1])
        cv.rectangle(frame, (left, top - round(1.5*labelSize[1])), (left + round(1.5*labelSize[0]), top + baseLine), (255, 255, 255), cv.FILLED)
        cv.putText(frame, area, (left, top), cv.FONT_HERSHEY_SIMPLEX, 0.75, (0,0,0), 1)
...
if (args.image):
    ...
    outputFile = args.video[:-4]+'out.mp4'
else:
    # Webcam input
